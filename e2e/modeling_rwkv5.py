...
def rwkv_linear_attention_v5_cpu(
    ...
    for t in range(T):
        ...
        vt = value[:, :, t : t + 1, :]
        at = kt @ vt
        out[:, t] = (rt @ (time_first * at if state is None else (time_first * at + state))).squeeze(2)
        with torch.no_grad():
            state = at + (time_decay if state is None else (time_decay * state))
...
class RwkvSelfAttention(nn.Module):
    ...
    def forward(self, hidden, state=None, use_cache=False, seq_mode=True):
        ...
        if layer_state is not None and state is not None:
